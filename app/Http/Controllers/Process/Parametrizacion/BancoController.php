...
class BancoController extends Controller{
    ...
    public function par_data_Banco_DL(Request $request){
        $draw = $request['draw'];
        ...
        $sqlFrom = "FROM par_documento_legal_banco Where "
                .$sqlAdicional;
        ...
        $PAR_BANCOS_DOCUMENTOS_LEGALES_ESTADO = DB::SELECT("SELECT p.Nombre, p.Descripcion, p.Pantalla "
                    . "FROM Usuario u "
                    . "INNER JOIN Par_Perfi_Permiso pp on u.IdPerfil = pp.IdPerfil "
                    . "INNER JOIN Par_Permiso p on pp.IdPermiso = p.Id "
                    . "WHERE u.Token = '". session('keyUser')."' and p.nombre in ('PAR_BANCOS_DOCUMENTOS_LEGALES_ESTADO')");
        $PAR_BANCOS_DOCUMENTOS_LEGALES_EDITAR = DB::SELECT("SELECT p.Nombre, p.Descripcion, p.Pantalla "
                    . "FROM Usuario u "
                    . "INNER JOIN Par_Perfi_Permiso pp on u.IdPerfil = pp.IdPerfil "
                    . "INNER JOIN Par_Permiso p on pp.IdPermiso = p.Id "
                    . "WHERE u.Token = '". session('keyUser')."' and p.nombre in ('PAR_BANCOS_DOCUMENTOS_LEGALES_EDITAR')");

        foreach($Records['empRecords'] as $row){
            $permiso_estado = 0;
            $permiso_editar = 0;

            if( count($PAR_BANCOS_DOCUMENTOS_LEGALES_ESTADO) == 1 ){
                $permiso_estado = 1;
            }
            if( count($PAR_BANCOS_DOCUMENTOS_LEGALES_EDITAR) == 1 ){
                $permiso_editar = 1;
            }
            $data[] = array(
              "Num"=>$i,
              ...
              "Nombre"=>$row->Nombre,
              "PAR_BANCOS_DOCUMENTOS_LEGALES_ESTADO"=>$permiso_estado,
              "PAR_BANCOS_DOCUMENTOS_LEGALES_EDITAR"=>$permiso_editar,
              "Estado"=>$row->Estado
            );
           $i++;
        }
    ...
    public function par_data_Banco_TM(Request $request){
        $draw = $request['draw'];
        ...
        $sqlFrom = "FROM Par_Clasificacion_Banco Where "
                .$sqlAdicional;
        ...
        $PAR_BANCOS_TIPO_MOVIMIENTOS_ESTADO = DB::SELECT("SELECT p.Nombre, p.Descripcion, p.Pantalla "
                    . "FROM Usuario u "
                    . "INNER JOIN Par_Perfi_Permiso pp on u.IdPerfil = pp.IdPerfil "
                    . "INNER JOIN Par_Permiso p on pp.IdPermiso = p.Id "
                    . "WHERE u.Token = '". session('keyUser')."' and p.nombre in ('PAR_BANCOS_TIPO_MOVIMIENTOS_ESTADO')");
        $PAR_BANCOS_TIPO_MOVIMIENTOS_EDITAR = DB::SELECT("SELECT p.Nombre, p.Descripcion, p.Pantalla "
                    . "FROM Usuario u "
                    . "INNER JOIN Par_Perfi_Permiso pp on u.IdPerfil = pp.IdPerfil "
                    . "INNER JOIN Par_Permiso p on pp.IdPermiso = p.Id "
                    . "WHERE u.Token = '". session('keyUser')."' and p.nombre in ('PAR_BANCOS_TIPO_MOVIMIENTOS_EDITAR')");

        foreach($Records['empRecords'] as $row){
            $permiso_estado = 0;
            $permiso_editar = 0;

            if( count($PAR_BANCOS_TIPO_MOVIMIENTOS_ESTADO) == 1 ){
                $permiso_estado = 1;
            }
            if( count($PAR_BANCOS_TIPO_MOVIMIENTOS_EDITAR) == 1 ){
                $permiso_editar = 1;
            }
            $data[] = array(
              "Num"=>$i,
              "Hash"=> (new Controller)->CodeEncrypt($row->Id),
              "Nombre"=>$row->Nombre,
              "PAR_BANCOS_TIPO_MOVIMIENTOS_ESTADO"=>$permiso_estado,
              "PAR_BANCOS_TIPO_MOVIMIENTOS_EDITAR"=>$permiso_editar,
              "Estado"=>$row->Estado
            );
           $i++;
        }
    ...
    public function index(){
        if ( session('Time') > date("Y-m-d H:i:s") ){
            return view('process.parametrizacion.banco');
        }else{
    ...
    public function par_DocLegal(){
        ...
        if( DB::INSERT("INSERT into par_documento_legal_banco (Nombre,Estado,IdUsuario,Fecha)"
                . "values ('".addslashes($Credentials['ParDocLegal'])."', 1, ".$IdUsuario[0]->IdUsuario.",'".date("Y-m-d H:i:s")."');")){

            $id = DB::getPdo()->lastInsertId();
            $Query = "INSERT into par_documento_legal_banco (Nombre,Estado,IdUsuario,Fecha)"
                . "values ('".addslashes($Credentials['ParDocLegal'])."', 1, ".$IdUsuario[0]->IdUsuario.",'".date("Y-m-d H:i:s")."');";

            DB::INSERT("INSERT INTO Auditoria (IdConcepto,Tipo,Query,Accion,IdUsuario,Fecha) values ("
                    . "".$id.",'PAR_CREACION_DOCLEGAL_BANCO','".addslashes($Query)."','INSERT',".$IdUsuario[0]->IdUsuario.",'".date("Y-m-d H:i:s")."');");
    ...
    public function par_DocLegalEstado(){
        ...
        $Estado = DB::SELECT("SELECT Estado FROM par_documento_legal_banco WHERE Id = '".(new Controller)->CodeDescrypt($Credentials['Hash'])."';");
        $Query = "";
        if( $Estado[0]->Estado == 0 || $Estado[0]->Estado == 1 ){
            ...
            if(DB::UPDATE("UPDATE par_documento_legal_banco set Estado = '$n' WHERE Id = '".(new Controller)->CodeDescrypt($Credentials['Hash'])."';")){

                $IdUsuario = DB::SELECT("SELECT IdUsuario FROM Usuario WHERE Token = '".session('keyUser')."'");
                $id = (new Controller)->CodeDescrypt($Credentials['Hash']);
                $Query = "UPDATE par_documento_legal_banco set Estado = '$n' WHERE Id = '".(new Controller)->CodeDescrypt($Credentials['Hash'])."';";
                $Query = str_replace("'","~",$Query);
                DB::INSERT("INSERT INTO Auditoria (IdConcepto,Tipo,Query,Accion,IdUsuario,Fecha) values ("
                        . "".$id.",'PAR_EDICION_ESTADO_DOCLEGAL_BANCO','".addslashes($Query)."','UPDATE',".$IdUsuario[0]->IdUsuario.",'".date("Y-m-d H:i:s")."');");
    ...
    public function par_DocLegalEdit(){
        ...
        DB::UPDATE("UPDATE par_documento_legal_banco set Nombre = '".addslashes($Credentials['ParDocLegal'])."' "
                . " WHERE Id = '".(new Controller)->CodeDescrypt($Credentials['Hash'])."';");

            $IdUsuario = DB::SELECT("SELECT IdUsuario FROM Usuario WHERE Token = '".session('keyUser')."'");
            $id = (new Controller)->CodeDescrypt($Credentials['Hash']);
            $Query = "UPDATE par_documento_legal_banco set Nombre = '".addslashes($Credentials['ParDocLegal'])."' "
                . " WHERE Id = '".(new Controller)->CodeDescrypt($Credentials['Hash'])."';";

            DB::INSERT("INSERT INTO Auditoria (IdConcepto,Tipo,Query,Accion,IdUsuario,Fecha) values ("
                    . "".$id.",'PAR_EDICION_DOCLEGAL_BANCO','".addslashes($Query)."','UPDATE',".$IdUsuario[0]->IdUsuario.",'".date("Y-m-d H:i:s")."');");
